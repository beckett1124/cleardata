...
function join_files()
{
    ...
    if [ ${file_type} == "err" ];then
        join_file_path_name="${des_path}/*/*/err*_${topic}_${start_time}_des_err_*"
    else
        join_file_path_name="${des_path}/*/*/*rawdata_${topic}_${start_time}_*"
    fi


    des_files=(`ls ${join_file_path_name}`)
    ...
    if [ ${#des_files[@]} -ge 1 ];then
        ...
        rm ${join_file_path_name}  2>/dev/null
    fi
}
...
#    cat ${file[*]} | python format/format_${topic}.py ${start_time}
    cat ${file[*]} | python run_wash_new_process.py ${topic} ${start_time}
