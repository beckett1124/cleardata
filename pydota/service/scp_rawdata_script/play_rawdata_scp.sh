#!/bin/sh
# playrawdata日志检验的topic名称
other_vv_topics=("mpp_vv_pcclient mpp_vv_padweb mpp_vv_ott ott_vv_41 ott_vv_44 ott_vv_311_20151012 mpp_vv_win10client_511_20151030 macclient_vv_811_20151210 pcweb_1110_20151223 phonem_vod_4110_20160317")
...
other_vv_cp_host="10.100.1.141"
mobile_vv_cp_host="10.100.5.81"

if [ $# -ge 1 ];then
    start_time=$1
else
    start_time=`date --date="$DATE - 3 hour" +%Y%m%d%H`
fi
...
echo "start scp playrawdata,date:${start_time}"
...
function cp_local_to_s3()
{
    ...
    if [[ ${des_topic} == "pcweb_1110_20151223" ]];then
        return 0
    fi
    ...
    for topic in ${topics};do
        ...
        if [ ${cp_msg} -eq 0 ];then
            ...
            do
                ...
                # scp to nfs
                nfs_file=${fil/\/home\/dota\/data/\/data\/nfs\/dota}
                sudo mkdir -p ${nfs_file%/*}
...
scp_des_to_local "${other_vv_topics[*]}"  ${other_vv_cp_host}
scp_des_to_local "${mobile_vv_topics[*]}"  ${mobile_vv_cp_host}


if [ ${cp_flag} -eq 0 ];then
    touch ${local_des_path}/${sub_year}/${sub_month}/.${start_time}_playrawdata.done
    touch ${nfs_path}/${sub_year}/${sub_month}/.${start_time}_playrawdata.done

    for topic in ${other_vv_topics};do
        index_err_log ${topic}
    done

    for topic in ${mobile_vv_topics};do
        index_err_log ${topic}
    done

fi
echo "end scp playrawdata,date:${start_time}"