#!/bin/sh
# pvrawdata日志检验的topic名称
pv_topics=("pcweb_pv ott_pv msite_pv")
#topics=("mpp_vv_pcweb")
local_host="10.100.5.4"
pv_cp_host="10.100.1.143"
...
echo "start scp pvrawdata,date:${start_time}"
...
scp_des_to_local "${pv_topics[*]}"  ${pv_cp_host}


if [ ${cp_flag} -eq 0 ];then
    touch ${local_des_path}/${sub_year}/${sub_month}/.${start_time}_pvrawdata.done


    for topic in ${pv_topics};do
        index_err_log ${topic}
    done

fi
echo "end scp pvrawdata,date:${start_time}"