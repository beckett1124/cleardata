#!/bin/sh
# daurawdata日志检验的topic名称
dau_topics=("dau_ott dau_ott_41 dau_ott_44 dau_ott_3111 dau_ott_340")
#topics=("mpp_vv_pcweb")
local_host="10.100.5.4"
dau_cp_host="10.100.1.143"
...
dau_rawdata_check_path="/home/dota/data/dau_rawdata_check/"
...
echo "start scp daurawdata,date:${start_time}"
...
function scp_des_to_local()
{
    ...
    for topic in ${topics};do
        ...
        # des scp
        cp_file_name="*_daurawdata_${topic}_${start_time}00"
...
scp_des_to_local "${dau_topics[*]}"  ${dau_cp_host}


if [ ${cp_flag} -eq 0 ];then
    touch ${local_des_path}/${sub_year}/${sub_month}/.${start_time}_daurawdata.done


    for topic in ${dau_topics};do
        index_err_log ${topic}
    done

fi
echo "end scp daurawdata,date:${start_time}"