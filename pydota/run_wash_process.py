...
from pydota.format.pydota_common_new import write_to_file
import ConfigParser
...
def work_func(line_list):
    for line in line_list:
        # if line_num[0] % 10000 == 0:
        #     print "ProcessID[%s]: line_num[%d]: cur_time[%s]" % (os.getpid(), line_num[0], time.time())
        #     print "ProcessID[%s]: line_num[%d]: run_time[%s]" % (os.getpid(), line_num[0], str(run_time_list[0]))

        # befor_time = time.time()
        format_client.processFormat(line)
        # run_time = time.time() - befor_time

        # run_time_list[0] = run_time_list[0] + run_time
        # line_num[0] += 1
