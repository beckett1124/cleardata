...
# encoding: utf-8
"""
Brief  : format mpp_vv_pcweb class
...
class MppVVPcWebFormat(FormatBase):
    """
    mpp_vv_pcweb
    """
    def __init__(self, recv_time, topic_name='mpp_vv_pcweb'):
        super(MppVVPcWebFormat, self).__init__(recv_time)
    ...
    def _wash_pt(self, log_key, log_dict):
        ...
        """
        if "bid" in log_dict.keys() and "tp" in log_dict.keys():
            if str(log_dict["bid"]) == '1' and str(log_dict["tp"]) == '1':
                return [0, '0']
        else:
...
if __name__ == '__main__':
    ...
    mpp_vv_pcweb_client = MppVVPcWebFormat(start_time)
    for line in fileinput.input(sys.argv[2:]):
        res = mpp_vv_pcweb_client.processFormat(line)
        # print res[0]
        # print res[1]
