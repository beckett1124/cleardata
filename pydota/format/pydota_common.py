...
def __genOutputFileName(log_time, topic, start_time, data_type):
    ...
    if "err" in data_type:
        file_str = '{0}/{1}/{2}/err_{3}_{4}_{5}_{6}'.format(str(output_path), log_time[0:4],
                                                log_time[4:6], log_time, topic, start_time, data_type)
    elif "des" == data_type:
        if "_pv" in topic:
            ...
            file_str = '{0}/{1}/{2}/{3}_daurawdata_{4}_{5}'.format(str(output_path), log_time[0:4],
                                                    log_time[4:6], log_time, topic, start_time)
        else:
