...
class _pydotalog(object):
    def __init__(self, logger_name="pydotalog", tag_name=""):
    ...
    def _handle_msg(self, msg, args, kwargs):
        if args:
            msg = msg % args
        msg = self.__enc_excinfo(msg, kwargs)

        msg = self.__indent_msg(msg.rstrip(), args)
        return msg

    def __enc_excinfo(self, msg, kwargs):
        """@summary: 将kwargs中的exc_info，merge到msg中
        @attention: exc_info为traceback三元组时，不进行转码"""
        if "exc_info" not in kwargs or not kwargs["exc_info"] or isinstance(kwargs["exc_info"], tuple):
            # kwargs不存在exc_info
            # 或者 exc_info为False
            # 或者 exc_info为tuple
            return msg
...
# matslog 全局句柄，默认设置屏幕输出
pydotalog = LogProxy(_pydotalog())
pydotalog.init_stream_handler()


def _test():
    pydotalog.init_logger("./test.log")

    pydotalog.debug("debug 日志")
    pydotalog.info("info %s 日志\n换行")
    try:
        raise Exception, "except 异常"     # this is a 异常注释 !
    except Exception:
        pydotalog.warning("warning %s %s", "日志", "xxxx",  exc_info=True)
    pydotalog.diagnose("error %s日志", "格式字符串")
    pydotalog.critical("critical 日志")
