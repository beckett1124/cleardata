...
for topic in ${topics[*]};
do
    filename="${start_time}*rawdata_${topic}_20*"
    local_file_path=${des_path}/${sub_year}/${sub_month}/
    ...
    if [[ ${topic} == "mpp_vv_ott" || ${topic} == "ott_vv_311_20151012" || ${topic} == "ott_vv_41" || ${topic} == "ott_vv_44" ]];then
        if [[ ${dm_fact} == "vv_fact" ]];then
            dm_done=${dm_done_path}/${sub_year}/${sub_month}/${dm_fact}_${topic}_${start_hour}.done
        fi
    fi
    touch ${dm_done}
