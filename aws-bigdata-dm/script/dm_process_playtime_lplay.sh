...
pydota_root_split_des="$HOME/data/des_split"
...
# 将记录分发到各自的小时&pt文件
function split_file_to_hour_pt()
{
    split_time=$1
    split_topic_name=$2
    split_dm_fact=$3

    local_file_path="${des_path}/${split_topic_name}/${start_hour:0:4}/${start_hour:4:2}/${start_hour:6:2}/${start_hour:8:2}"
    split_file_path="${pydota_root_split_des}/${split_topic_name}/${start_hour:0:4}/${start_hour:4:2}/${start_hour:6:2}/${start_hour:8:2}"

    split_files=`ls ${local_file_path}/*|grep -v "_SUCCESS"`

    if [ ! -d ${split_file_path} ]; then
            mkdir -p ${split_file_path}
    fi

    echo "`date +%Y%m%d%H%M%S` hour file is ${split_time}"

    if [ ${#split_files[@]} -ge 1 ];then
        cat ${split_files[@]} |awk -F "," -v split_topic_name=${split_topic_name} -v path=${split_file_path} -v recv_time=${split_time} '{print > path"/tmp_"substr($2,1,10)"_playtimerawdata_"$16"_"split_topic_name"_"recv_time}'
    fi
	...
	do
        ...
        file_path_tmp=${pydota_root_split_des}/${split_topic_name}/${time_file_name:0:4}/${time_file_name:4:2}/${time_file_name:6:2}/${time_file_name:8:2}
...
function wash_dm_and_sync()
{
    ...
    if [ ${#files_tmp[@]} -ge 1 ];then
        ...
        cat ${files_tmp[*]} | python app.py ${topic_tmp} ${dm_fact_tmp} ${start_hour}
    ...
    # 生成done文件，并上传到s3
    dm_done=${dm_done_path}/${sub_year}/${sub_month}/${topic_tmp}_${start_hour}.done

    touch ${dm_done}
    err_msg=`aws s3 cp ${dm_done} ${pydota_s3_done}/ 2>&1`
    err_num=$?
    if [[ ${err_num} -ne 0 ]];then
        send_error_mess "[${start_hour}]${topic_tmp} des_done 文件上传s3失败. err_msg:[${err_msg}]"
        exit 1
    fi
...
do
    ...
    split_file_to_hour_pt ${start_hour} ${topic} ${dm_fact}

    split_file_path="${pydota_root_split_des}/${topic}/${start_hour:0:4}/${start_hour:4:2}/${start_hour:6:2}/${start_hour:8:2}"

    # 点播 pt ＝ 0
    files=(`ls ${split_file_path}/${start_hour}_playtimerawdata_0_${topic}_20*`)
    ...
    do
        if [[ ${split_topic} == ${topic} ]];then
            files=(`ls ${split_file_path}/${start_hour}_playtimerawdata_2_${topic}_20* ${split_file_path}/${start_hour}_playtimerawdata_5_${topic}_20*`)
            dm_fact="lplaytime_fact"
            topic=${topic}_lplay
            wash_dm_and_sync ${topic} ${dm_fact} "${files[*]}"
