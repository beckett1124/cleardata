...
from data_class.media_cms_plid_name_map import mpp_cms_pid_name_map_client
from data_class.media_cms_playlist_vid import media_cms_playlist_vid_map_client


class MediaPidName(rule):
    """
    :换算规则
    """

    def __init__(self):
        super(MediaPidName, self).__init__()
    ...
    def convert(self, field):
        ...
        if str(field[0]).isdigit() and int(field[0]) > 100000000:
            ...
            if _res[0] == 0:
                ...
                return _res
        _res_pid = mpp_cms_id_map_client.get_p_lid_by_vid(field)

        if _res_pid[0] == 0:
            return mpp_cms_pid_name_map_client.get_p_name_by_pid([_res_pid[1]])
        else:
            return _res_pid
